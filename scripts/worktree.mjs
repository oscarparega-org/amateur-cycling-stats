...
export function deriveWorktreeIdentity(root) {
  ...
  return {
    id: `${name}-${hash}`,
    projectName: `amateur-cycling-stats-${name}-${hash}`.slice(0, 63).replace(/-$/, ''),
    hostname: `${name}-${hash}.localhost`,
    databaseName: `acs_${hash}`
  };
}
...
function gitCommonDirectory(root) {
  return resolve(root, gitOutput(root, ['rev-parse', '--git-common-dir']));
}

async function withAllocationLock(root, action) {
  const lockPath = join(tmpdir(), `acs-worktree-${shortHash(gitCommonDirectory(root), 12)}.lock`);
...
function readConfig(root, required = true) {
  ...
  return validateWorktreeConfig(root, parseEnv(readFileSync(envPath, 'utf8')), envPath);
}
...
async function initialize(root) {
  ...
  return withAllocationLock(root, async () => {
    ...
    const identity = deriveWorktreeIdentity(root);
    const ports = await selectPortSlot(resolve(root), usedWorktreePorts(root));
    const databaseUser = 'acs';
    ...
    const config = {
      ...
      DATABASE_URL: `postgresql://${databaseUser}:${databasePassword}@localhost:${ports.postgres}/${identity.databaseName}`,
...
function runtimeEnvironment(config) {
  return { ...process.env, ...config, API_URL: `http://127.0.0.1:${config.API_PORT}` };
}

function compose(root, config, args) {
...
function startChild(label, executable, args, root, env) {
  ...
  return spawn(executable, args, { cwd: root, env, stdio: 'inherit' });
}
...
async function develop(root) {
  ...
  if (!available.every(Boolean)) {
    ...
    throw new Error(`Application port(s) already in use: ${busy.join(', ')}. Stop the existing worktree process.`);
  }
...
function down(root, removeConfig = false) {
  ...
  if (config) compose(root, config, ['down', '--volumes', '--remove-orphans']);
  else {
    console.log(`[worktree] no ${envFilename}; cleaning derived Compose project ${target.projectName}`);
...
function assertClean(targetRoot) {
  ...
  if (dirty) {
    throw new Error(
      `Refusing to remove a dirty worktree:\n${dirty}\nCommit/stash the changes, or pass --force intentionally.`
    );
  }
}
...
function removeWorktree(sourceRoot, rawArgs) {
  ...
  if (targetRoot === resolve(sourceRoot)) throw new Error('Run wt:remove from a different worktree than the target.');
...
function usage() {
  console.log(
    'Usage: node scripts/worktree.mjs <command>\n\n' +
      ...
      '  dev                    Start PostgreSQL, migrate, and run with hot reload\n' +
