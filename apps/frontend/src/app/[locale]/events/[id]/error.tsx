...
export default function EventError({ reset }: { error: Error; reset: () => void }) {
  ...
  return (
    <main className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-3xl place-items-center px-5 py-16 text-center sm:px-8">
      <div role="alert">
        <h1 className="text-5xl font-semibold leading-none text-[#102a43]">{t('No pudimos cargar el evento')}</h1>
        <p className="mt-5 text-lg text-slate-600">{t('Revisa tu conexión e intenta de nuevo.')}</p>
        <button
