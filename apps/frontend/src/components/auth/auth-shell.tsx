    ...
    <main className="grid min-h-screen lg:grid-cols-[minmax(22rem,0.9fr)_minmax(32rem,1.1fr)]">
      <aside className="timing-grid relative hidden overflow-hidden bg-[#102a43] p-12 text-white lg:flex lg:flex-col lg:justify-between xl:p-16">
        ...
        <div className="relative z-10 max-w-md pb-14">
          <div className="mb-8 flex items-center gap-3 text-sm text-blue-100">
            <span className="h-px w-12 bg-[#f97316]" />
            {t('Cada segundo cuenta')}
          </div>
          ...
          <p className="mt-6 max-w-sm text-lg leading-7 text-blue-100">
            {t('Resultados claros para ciclistas, equipos y organizadores.')}
          </p>
        </div>
      ...
      <section className="flex items-center justify-center bg-white px-5 py-12 sm:px-10 lg:px-16">
        <div className="w-full max-w-md">
          ...
          <h1 className="text-4xl font-semibold tracking-tight text-[#102a43] sm:text-5xl">{t(title)}</h1>
          {intro ? <p className="mt-3 max-w-sm text-base leading-7 text-slate-600">{t(intro)}</p> : null}
