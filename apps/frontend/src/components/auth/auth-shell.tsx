    ...
    <main className="grid min-h-screen lg:grid-cols-[minmax(22rem,0.9fr)_minmax(32rem,1.1fr)]">
      <aside className="timing-grid relative hidden overflow-hidden bg-[#102a43] p-12 text-white lg:flex lg:flex-col lg:justify-between xl:p-16">
        ...
        <div className="relative z-10 max-w-md pb-14">
          <div className="mb-8 flex items-center gap-3 text-sm text-blue-100"><span className="h-px w-12 bg-[#f97316]" />Cada segundo cuenta</div>
          <p className="display-font text-6xl font-semibold leading-[0.88] tracking-tight xl:text-7xl">Tu carrera.<br />Tus números.</p>
          <p className="mt-6 max-w-sm text-lg leading-7 text-blue-100">Resultados claros para ciclistas, equipos y organizadores.</p>
        </div>
        <div className="route-profile absolute inset-x-0 bottom-0 h-64 bg-[#2563eb] opacity-75" aria-hidden="true" />
        <div className="route-profile absolute inset-x-0 bottom-0 h-48 translate-y-10 bg-[#f97316] opacity-90" aria-hidden="true" />
      </aside>
      <section className="flex items-center justify-center bg-white px-5 py-12 sm:px-10 lg:px-16">
        <div className="w-full max-w-md">
          ...
          <p className="mt-3 max-w-sm text-base leading-7 text-slate-600">{intro}</p>
